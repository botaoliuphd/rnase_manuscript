...
#### Similar for other samples shown in the manuscript ###
#### Base composition files for other samples could be downloaded as the processed files from GSE116233 # 

base <- read.table("qc_sample_wt_rpf_batch1_rep1_reads_overlap_CDS.base",sep="\t",header=T)
...
gc_perc <- mean(base_norm_gc [10:20])

gc_perc <- format(round(gc_perc, 2), nsmall = 2)

pdf("qc_sample_wt_rpf_batch1_rep1_reads_overlap_CDS.base.pdf",width=6,height=6)
par(pin=c(4,3.3))
