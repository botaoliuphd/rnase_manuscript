...
base <- read.table("ipsc_neuron_gc.txt",sep="\t",header=T)
log2_rna = log(base[,"rna"])/log(2)

pdf("ipsc_neuron_gc_scatter.pdf",width=7,height=6)
par(pin=c(4,3.3))
  
r1 <- format(round(cor(log2_rna, base[,"gc"]),3), nsmall = 2)
r2 <- format(round(cor(log2_rna, base[,"length"]),3), nsmall = 2)

plot(log2_rna, base[,"gc"], axes=FALSE, pch=17,cex.lab=1.5,cex.axis=1.5,col="black", xlab="", ylab="",xlim=c(-2,5),ylim=c(49,56))
...
plot(log2_rna, base[,"length"], axes=FALSE, pch=17,cex.lab=1.5,cex.axis=1.5,col="red", xlab="", ylab="",xlim=c(-2,5),ylim=c(27,34))
